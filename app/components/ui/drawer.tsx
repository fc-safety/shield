...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal
    container={
      typeof document !== "undefined" ? document.querySelector("#dialog-portal") : undefined
    }
  >
    <DrawerOverlay />
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "bg-background fixed inset-x-0 bottom-0 z-50 mt-12 flex h-auto flex-col rounded-t-[10px] border",
        className
      )}
      {...props}
    >
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn("grid gap-1.5 px-4 py-2 text-center sm:py-4 sm:text-left", className)}
    {...props}
  />
);
